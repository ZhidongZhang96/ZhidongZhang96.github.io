---
title: "Linux davfs2搭载Webdav协议网盘"
date: 2023-12-05
# 自定义 URL 示例：
...
- sh脚本和其调用的C语言程序都含有`sudo`命令，在配置visudo后依旧存在权限问题。
...
- 安装相关依赖libxml2：
    ...
    <aside>
    ⚠️ 报错`*Error: GPG check FAILED*`，表示GPG检查不通过，在命令后添加``--nogpgcheck``规避检查
    </aside>
...
- 编辑`/usr/local/etc/davfs2/secrets`文件，添加webdav访问的凭证，格式如下：之后更改设置权限，确保只有root权限才可访问：
...
- 网盘和卸载
    ...
    ```bash
    # 添加环境变量export PATH=$PATH:/usr/local/sbin
    # 挂载
    mount.davfs <WebDav_URL> <mount_point>
    # 卸载
    umount <mount_point>
...
- 执行`visudo`命令，修改`sudoers`文件，添加下行：使得davfs2用户能够无密码执行该文件
