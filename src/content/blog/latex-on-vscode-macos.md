...
Click the gear icon in the bottom left, and click `Settings` to open the settings page, where we can open the `settings.json` by clicking the "Open settings file" button in the top right. Then we can edit the `settings.json` file directly by adding the following lines:

```json
{
    // LaTeX
    // Don't compile automatically after saving
...
[^2]: [LaTeX之一：macos上latex环境搭建（homebrew安装+vscode配置+ MacTex）](https://blog.csdn.net/zhiaidaidai/article/details/140440448)
